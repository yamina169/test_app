// WCAG: Icon-only buttons must have an accessible name via aria-label or sr-only text.
...
const buttonVariants = cva(
  ...
  ].join(" "),
  {
    variants: {
      ...
      size: {
        // WCAG 2.5.5: ensure adequate target size (prefer >= 44x44 for primary actions in design QA)
        ...
        icon: "h-10 w-10",
        "icon-xs": "h-8 w-8 rounded-md [&_svg:not([class*='size-'])]:size-3",
        "icon-sm": "h-9 w-9",
        "icon-lg": "h-11 w-11",
      },
    },
