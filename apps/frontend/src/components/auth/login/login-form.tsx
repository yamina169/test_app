...
import { useLoginWithEmail } from "@/application/hooks/useAuth";
import {
...
export function LoginForm() {
  ...
  const { submit, isPending, error: authError } = useLoginWithEmail();
  ...
  function focusField(key: keyof LoginWithEmailValues) {
    document.getElementById(loginFieldIds[key])?.focus();
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) return;
    await submit({ email: res.values.email, password: res.values.password });
  }
